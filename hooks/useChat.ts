...
export default function useChat() {
    ...
    const subscribeToCurrentParty = (userId: string) => {
        const userChannel = supabase
            ...
            .on("postgres_changes", { event: "UPDATE", schema: "public", table: "users", filter: `user_id=eq.${userId}` }, payload => {
                ...
                if (updatedCurrentParty === null) setNoRoomMessage(true), setRoomId(null);
                else if (updatedCurrentParty !== roomId) setRoomId(updatedCurrentParty), setNoRoomMessage(false);
            })
            .subscribe();
    ...
    return { messages, newMessage, noRoomMessage, setNewMessage, handleSendMessage };
}
